...
func preparedNamedQuery(ctx context.Context, tx preparer, query string, args ...interface{}) error {
	...
	case 1:
		if isSlice(args[0]) {
			return selectSwitch(ctx, stmt, args[0], nil)
		}
		return exec(ctx, stmt, args[0])
	case 2:
		return selectSwitch(ctx, stmt, args[0], args[1])
	default:
...
func selectSwitch(ctx context.Context, stmt *sqlx.NamedStmt, target interface{}, input interface{}) error {
