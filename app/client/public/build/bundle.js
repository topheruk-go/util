...
(function () {
    ...
    const file = "app\\client\\src\\App.svelte";
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(a, file, 9, 2, 130);
    			add_location(p, file, 7, 1, 112);
    ...
    const formattedDate = (d) => `${d.getFullYear()}-${(d.getMonth() + 1)
    .toString()
    ...
    const file_1 = "app\\client\\src\\Form.svelte";
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			button.textContent = "Please save me...";
    			this.c = noop;
    			add_location(p, file_1, 34, 8, 861);
    			attr_dev(input, "type", "file");
    			attr_dev(input, "name", "file");
    			attr_dev(input, "accept", ".pdf");
    			add_location(input, file_1, 35, 8, 882);
    			add_location(label, file_1, 33, 4, 844);
    			attr_dev(button, "type", "submit");
    			add_location(button, file_1, 63, 4, 1580);
    			attr_dev(form, "enctype", "multipart/form-data");
    			add_location(form, file_1, 32, 0, 768);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	async function submit(e) {
    		...
    		//     body[k] = v;
    		// }
    		file = new FormData(this).get("file");

    		console.log(await file.arrayBuffer());
    		let json = JSON.stringify({ name, age, email, file });
    	...
    	Object.keys($$props).forEach(key => {
    		if (!~writable_props.indexOf(key) && key.slice(0, 2) !== '$$' && key !== 'slot') console_1.warn(`<signup-form> was created with unknown prop '${key}'`);
    	});
    	...
    	return [fs, submit, input_change_handler];
    }
