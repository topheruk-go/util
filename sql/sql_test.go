...
func TestQueryRow(t *testing.T) {
	...
	var b testcase
	err := QueryRowPSQL(db, func(row pgx.Row) error { return row.Scan(&b.Id, &b.Name, &b.Age) }, "SELECT * FROM testcase WHERE id = $1", 1)
	assert.Assert(t, err)
...
func TestQuery(t *testing.T) {
	...
	items, err := QueryPSQL(db, func(rows pgx.Rows, i *testcase) error {
		return rows.Scan(&i.Id, &i.Name, &i.Age)
	}, "SELECT * FROM testcase WHERE age > $1", 26)
...
func TestExec(t *testing.T) {
	...
	err := ExecPQSL(db, "INSERT INTO testcase (name,age) VALUES ($1,$2)", "Maxi", 31)
	assert.Assert(t, err)
}
